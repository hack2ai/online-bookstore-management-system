...
class AdminAuditServiceImplTest {
    ...
    void normalizesFiltersBeforeSearching() {
        ...
        when(auditEventRepository.search(eq("LOGIN_SUCCESS"), eq(7L), eq("req-123"), eq(pageable))).thenReturn((Page) result);

        Page<?> response = service.search(" LOGIN_SUCCESS ", 7L, " req-123 ", pageable);

        assertThat(response).isSameAs(result);
        verify(auditEventRepository).search(eq("LOGIN_SUCCESS"), eq(7L), eq("req-123"), eq(pageable));
    }
    ...
    void convertsBlankFiltersToNull() {
        AdminAuditServiceImpl service = new AdminAuditServiceImpl(auditEventRepository);
        Pageable pageable = PageRequest.of(0, 25);
        when(auditEventRepository.search(isNull(), isNull(), isNull(), eq(pageable)))
                .thenReturn(new PageImpl<>(List.of()));

        service.search("  ", null, "  ", pageable);

        verify(auditEventRepository).search(isNull(), isNull(), isNull(), eq(pageable));
    }
}